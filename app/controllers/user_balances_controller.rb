class UserBalancesController < ApplicationController
  before_action :set_user_balance, only: %i[ show edit update destroy ]

  # GET /user_balances or /user_balances.json
  def index
    @user_balances = UserBalance.all
  end

  # GET /user_balances/1 or /user_balances/1.json
  def show
  end
  ...
  # POST /user_balances or /user_balances.json
  def create
    @user_balance = UserBalance.new(user_balance_params)

    respond_to do |format|
      if @user_balance.save
        format.html { redirect_to user_balance_url(@user_balance), notice: "User balance was successfully created." }
  ...
  # PATCH/PUT /user_balances/1 or /user_balances/1.json
  def update
    respond_to do |format|
      if @user_balance.update(user_balance_params)
        format.html { redirect_to user_balance_url(@user_balance), notice: "User balance was successfully updated." }
  ...
  # DELETE /user_balances/1 or /user_balances/1.json
  def destroy
  ...
  private
    ...
    def user_balance_params
      params.require(:user_balance).permit(:amount, :currency, :user_id)
    end
end
